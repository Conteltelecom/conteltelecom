<%@ Page Title="RELATÓRIO DESCRITIVO" Language="vb" AutoEventWireup="false" MasterPageFile="~/Relatorios.master" CodeBehind="rel_descritivo.aspx.vb" Inherits="conteltelecom.rel_descritivo" %>
...
<asp:ObjectDataSource ID="ObjectDataSource1" runat="server" OldValuesParameterFormatString="original_{0}" SelectMethod="GetData" TypeName="conteltelecom.ds_relDescritivoTableAdapters.RelDiscritivoTableAdapter">
  <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSourceDescritivoPorLinhas" runat="server" ConnectionString="<%$ ConnectionStrings:conteltelecom %>" SelectCommand="SELECT CL_PLANOSxLINHAS_1.idLinhasPai_LI_LINHAS, CL_PLANOSxLINHAS_1.id_LI_LINHAS, CL_PLANOS_CLIENTE_1.desc_CL_PLANOS_CLIENTE, CL_PLANOS_CLIENTE_1.ordem_CL_PLANOS_CLIENTE, LI_LINHAS_2.numLinha_LI_LINHAS, LI_LINHAS_1.codLinha_LI_LINHAS, PS_CIDADES_1.desc_PS_CIDADES, PS_JURIDICA_1.CNPJ_PS_JURIDICA, PS_JURIDICA_1.razaosocial_PS_JURIDICA, 0 AS tipoinfo, SF_VL_SERVICO.vl_SF_VL_SERVICO, SF_VL_SERVICO.vlDesconto_SF_VL_SERVICO, SF_VL_SERVICO.desc_SF_VL_SERVICO, SF_VL_SERVICO.qtDias_SF_VL_SERVICO, CL_PLANOS_CLIENTE_1.valor_CL_PLANOS_CLIENTE, OP_PLANOS.desc_OP_PLANOS, '' AS minutos_SF_VL_USO, PS_PESSOA.desc_PS_PESSOA, SF_VL_SERVICO.id_SF_VL_SERVICO, SF_SERVICOS_FATURA_1.contaRetificada_SF_SERVICOS_FATURA FROM PS_JURIDICA AS PS_JURIDICA_1 INNER JOIN PS_CLIENTES AS PS_CLIENTES_1 ON PS_JURIDICA_1.id_PS_JURIDICA = PS_CLIENTES_1.id_PS_JURIDICA INNER JOIN PS_PESSOA ON PS_CLIENTES_1.codMatriz_PS_CLIENTES = PS_PESSOA.id_PS_PESSOA INNER JOIN OP_PLANOS INNER JOIN CL_PLANOS_CLIENTE AS CL_PLANOS_CLIENTE_1 INNER JOIN CL_PLANOSxLINHAS AS CL_PLANOSxLINHAS_1 ON CL_PLANOS_CLIENTE_1.id_CL_PLANOS_CLIENTE = CL_PLANOSxLINHAS_1.id_CL_PLANOS_CLIENTE INNER JOIN LI_LINHAS AS LI_LINHAS_2 ON CL_PLANOSxLINHAS_1.id_LI_LINHAS = LI_LINHAS_2.id_LI_LINHAS INNER JOIN LI_LINHAS AS LI_LINHAS_1 ON CL_PLANOSxLINHAS_1.idLinhasPai_LI_LINHAS = LI_LINHAS_1.id_LI_LINHAS INNER JOIN PS_CIDADES AS PS_CIDADES_1 ON LI_LINHAS_1.id_PS_CIDADES = PS_CIDADES_1.id_PS_CIDADES ON OP_PLANOS.id_OP_PLANOS = CL_PLANOS_CLIENTE_1.id_OP_PLANOS ON PS_CLIENTES_1.id_PS_CLIENTES = LI_LINHAS_1.id_PS_CLIENTES LEFT OUTER JOIN SF_VL_SERVICO INNER JOIN SF_SERVICOS_FATURA AS SF_SERVICOS_FATURA_1 ON SF_VL_SERVICO.id_SF_SERVICOS_FATURA = SF_SERVICOS_FATURA_1.id_SF_SERVICOS_FATURA ON CL_PLANOSxLINHAS_1.id_LI_LINHAS = SF_SERVICOS_FATURA_1.id_LI_LINHAS WHERE (SF_SERVICOS_FATURA_1.mesAnoRefereincia_SF_SERVICOS_FATURA = @mesAnoRefereincia_SF_SERVICOS_FATURA) AND (SF_SERVICOS_FATURA_1.codMatriz_PS_CLIENTES = @codMatriz_PS_CLIENTES) AND (SF_SERVICOS_FATURA_1.numSequencia_SF_SERVICOS_FATURA = 0) AND (CL_PLANOSxLINHAS_1.id_LI_LINHAS = @id_LI_LINHAS) UNION ALL SELECT CL_PLANOSxLINHAS.idLinhasPai_LI_LINHAS, CL_PLANOSxLINHAS.id_LI_LINHAS, CL_PLANOS_CLIENTE.desc_CL_PLANOS_CLIENTE, CL_PLANOS_CLIENTE.ordem_CL_PLANOS_CLIENTE, LI_LINHAS.numLinha_LI_LINHAS, LI_LINHAS_1.codLinha_LI_LINHAS, PS_CIDADES.desc_PS_CIDADES, PS_JURIDICA.CNPJ_PS_JURIDICA, PS_JURIDICA.razaosocial_PS_JURIDICA, 1 AS tipoinfo, SF_VL_USO.vlUso_SF_VL_USO AS vl_SF_VL_SERVICO, 0 AS vlDesconto_SF_VL_SERVICO, SF_TIPO_USO.desc_SF_TIPO_USO AS desc_SF_VL_SERVICO, '' AS qtDias_SF_VL_SERVICO, CL_PLANOS_CLIENTE.valor_CL_PLANOS_CLIENTE, OP_PLANOS_1.desc_OP_PLANOS, OP_OPERADORAS.desc_OP_OPERADORAS + ' -' + CAST(SF_VL_USO.minutos_SF_VL_USO AS VARCHAR(11)) AS minutos_SF_VL_USO, PS_PESSOA_1.desc_PS_PESSOA, SF_VL_USO.id_SF_VL_USO AS id_SF_VL_SERVICO, SF_SERVICOS_FATURA.contaRetificada_SF_SERVICOS_FATURA FROM OP_OPERADORAS INNER JOIN SF_SERVICOS_FATURA INNER JOIN SF_VL_USO ON SF_SERVICOS_FATURA.id_SF_SERVICOS_FATURA = SF_VL_USO.id_SF_SERVICOS_FATURA INNER JOIN SF_TIPO_USO ON SF_VL_USO.id_SF_TIPO_USO = SF_TIPO_USO.id_SF_TIPO_USO ON OP_OPERADORAS.id_OP_OPERADORAS = SF_VL_USO.id_OP_OPERADORAS RIGHT OUTER JOIN PS_JURIDICA INNER JOIN PS_CLIENTES ON PS_JURIDICA.id_PS_JURIDICA = PS_CLIENTES.id_PS_JURIDICA INNER JOIN PS_PESSOA AS PS_PESSOA_1 ON PS_CLIENTES.codMatriz_PS_CLIENTES = PS_PESSOA_1.id_PS_PESSOA INNER JOIN OP_PLANOS AS OP_PLANOS_1 INNER JOIN CL_PLANOS_CLIENTE INNER JOIN CL_PLANOSxLINHAS ON CL_PLANOS_CLIENTE.id_CL_PLANOS_CLIENTE = CL_PLANOSxLINHAS.id_CL_PLANOS_CLIENTE INNER JOIN LI_LINHAS ON CL_PLANOSxLINHAS.id_LI_LINHAS = LI_LINHAS.id_LI_LINHAS INNER JOIN LI_LINHAS AS LI_LINHAS_1 ON CL_PLANOSxLINHAS.idLinhasPai_LI_LINHAS = LI_LINHAS_1.id_LI_LINHAS INNER JOIN PS_CIDADES ON LI_LINHAS_1.id_PS_CIDADES = PS_CIDADES.id_PS_CIDADES ON OP_PLANOS_1.id_OP_PLANOS = CL_PLANOS_CLIENTE.id_OP_PLANOS ON PS_CLIENTES.id_PS_CLIENTES = LI_LINHAS_1.id_PS_CLIENTES ON SF_SERVICOS_FATURA.id_LI_LINHAS = CL_PLANOSxLINHAS.id_LI_LINHAS WHERE (SF_SERVICOS_FATURA.mesAnoRefereincia_SF_SERVICOS_FATURA = @mesAnoRefereincia_SF_SERVICOS_FATURA) AND (SF_SERVICOS_FATURA.codMatriz_PS_CLIENTES = @codMatriz_PS_CLIENTES) AND (SF_SERVICOS_FATURA.numSequencia_SF_SERVICOS_FATURA = 0) AND (CL_PLANOSxLINHAS.id_LI_LINHAS = @id_LI_LINHAS)">
        <SelectParameters>
            <asp:Parameter Name="mesAnoRefereincia_SF_SERVICOS_FATURA" />
            <asp:Parameter Name="codMatriz_PS_CLIENTES" />
